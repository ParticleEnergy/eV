...
namespace eV.Server
{
    public static class ServerSession
    {
        ...
        {
            var session = SessionDispatch.Instance.SessionManager.GetActiveSession(sessionId);
            return session != null && session.Send(data);

        }
        public static void SendGroup(string selfSessionId, string groupId, byte[] data)
        {
            var groups = SessionDispatch.Instance.SessionGroup.GetGroup(groupId);
            if (groups == null)
            {
            ...
            {
                var session = SessionDispatch.Instance.SessionManager.GetActiveSession(group.Value);
